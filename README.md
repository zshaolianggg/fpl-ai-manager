...
As of `3.0.0a6.post1`, OpenAI is no longer a production decision-maker. The optimizer and deterministic equivalence/flexibility policy choose the final plan, which is then validated before delivery. An optional compact AI call may explain complex decisions, but its schema contains prose only and it cannot alter the selected action.
